...
int port(String nom){
  ...
  else {Serial.println(F("ERREUR CRITIQUE : la fonction port a été appellée avec un nom de moteur inexistant !"));alerte(2, F("Moteur_inexistant"));}
  ...
  else {Serial.println(F("ERREUR CRITIQUE : l'un des moteur a été assigné à un port inexistant !"));alerte(2,F("Port_inexistant"));}
}
...
int angle(byte valeur, String type){
  ...
  else {Serial.println(F("ERREUR CRITIQUE : la fonction angle a été appellée avec un type de moteur inexistant !"));alerte(2,F("Servo_inexistant"));}
  ...
  //debug(11,F("angle demande "),valeur);
  //debug(11, type ,-99);
  //debug(11,F("minimum "), minimum);
  //debug(11,F("maximum "), maximum);
  //debug(11,F("equivalent "),a_return);
  return(a_return);
}
...
void baie_gauche(int statut){
  ...
  if (statut == 2){
    ...
    servo(F("Servo_baie_gauche_gauche"), 60);
  } else if (statut == 0){
    baie_gauche_etat = 0;
...
void baie_arriere(int statut){
  ...
  if (statut == 2){
    ...
    servo(F("Servo_baie_arriere_gauche"), 35);
  } else if (statut == 0){
    baie_arriere_etat = 0;
...
void bras1_haut(){servo(F("Servo_levage1"),87);}
