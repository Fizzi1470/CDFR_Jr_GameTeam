...
void gestion_BAU(){
  if (digitalRead(53) == 1) {BAU = 1; identifiant_sequence = 0; sequence_terminee = 1;digitalWrite(13,0);}
...
void loop() {
  ...
  if (SwitchPro.connected() && lastMessageCounter != SwitchPro.getMessageCounter()) {
   ...
   if (SwitchPro.getButtonClick(CAPTURE))rien();
    ...
    if (SwitchPro.getButtonClick(A))baie_droite(-1);
  if (SwitchPro.getButtonClick(B))baie_arriere(-1);
  if (SwitchPro.getButtonClick(X))baie_avant(-1);
  if (SwitchPro.getButtonClick(Y))baie_gauche(-1);

    if (SwitchPro.getButtonClick(L))rien();
    if (SwitchPro.getButtonPress(R))rien();
  ...
  //if (Serial1.overflow()) Serial.println("ALED");
  gestion_magneto();
  gestion_score();
  gestion_sequences();
